...
import numpy as np
import random
from src.configs import build_config
from src.utils import setup_seed
from src.log import get_logger

from src.model import TimidModel
from src.dataset import *

from src.train import train_func
from src.test import test_func
from src.infer import infer_func
import argparse
import copy
...
def train(model, train_loader, test_loader, gt, logger, model_mode):
    if not os.path.exists(cfg.save_dir):
        os.makedirs(cfg.save_dir)

    criterion = torch.nn.BCELoss() if model_mode == 4 else torch.nn.BCEWithLogitsLoss() 
    ...
    initial_auc, n_far = test_func(test_loader, model, gt, cfg.dataset, model_mode)
    ...
    for epoch in range(cfg.max_epoch):
        loss1, loss2 = train_func(train_loader, model, optimizer, criterion, criterion2, model_mode,cfg.lamda)
        scheduler.step()

        auc, far = test_func(test_loader, model, gt, cfg.dataset, model_mode)
        if auc  >= best_metric:
            best_auc = auc
            auc_far = far
            best_model_wts = copy.deepcopy(model.state_dict())
        ...
        logger.info('[Epoch:{}/{}]: loss1:{:.4f} loss2:{:.4f} | PR:{:.4f} AR:{:.5f}'.format(
...
def main(cfg, args):
    ...
    elif cfg.dataset == 'ucf2':
        train_data = UCFDataset2(cfg, test_mode=False)
        test_data = UCFDataset2(cfg, test_mode=True)
    ...
    train_loader = DataLoader(train_data, batch_size=cfg.train_bs, shuffle=True,
                              num_workers=cfg.workers, pin_memory=True) if train_data is not None else None

    test_loader = DataLoader(test_data, batch_size=cfg.test_bs, shuffle=False,
                             num_workers=cfg.workers, pin_memory=True)

    model = TimidModel(cfg, mode=args.model_mode)
    gt = np.load(cfg.gt, allow_pickle=True)
    ...
    if args.mode == 'train':
        ...
        train(model, train_loader, test_loader, gt, logger, args.model_mode)

    elif args.mode == 'infer':
        logger.info('Test Mode')
        if args.ckpt_path is not None:
            load_checkpoint(model, args.ckpt_path, logger)
        else:
            logger.info('infer from random initialization')
        infer_func(model, test_loader, gt, logger, cfg, args.model_mode)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_mode', default=1, type=int, help='model mode')
    parser.add_argument('--ckpt_path', default=None, help='path to checkpoint')
