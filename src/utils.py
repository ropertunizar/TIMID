...
def chunk_to_label(chunk_labels, num_chunks, chunk_len):
    """ Assuming that labels are 0,1 """
    # count 0's and 1's in the vector
    # ensure input is a torch tensor
    if not torch.is_tensor(chunk_labels):
        chunk_labels = torch.from_numpy(np.array(chunk_labels))
    if not chunk_labels.is_cuda:
        chunk_labels = chunk_labels.cuda()

    labels = torch.zeros(0).cuda()
    for i in range(num_chunks):
        start = i * chunk_len
        end = start + chunk_len
        chunk = chunk_labels[start:end]
        zeros = torch.sum(chunk == 0)
        ones = torch.sum(chunk == 1)
        if ones >= zeros:
            labels = torch.cat((labels, torch.tensor([1]).cuda()))
        else:
            labels = torch.cat((labels, torch.tensor([0]).cuda()))
    return labels


def gt_complete_to_gt_chunk(gt_complete, chunk_len):
    # Return a CPU numpy array representing chunk-level labels
    arr = np.array(gt_complete)
    num_chunks = len(arr) // chunk_len
    gt_chunk = np.zeros(num_chunks, dtype=np.int64)
    for i in range(num_chunks):
        start = i * chunk_len
        end = start + chunk_len
        chunk = arr[start:end]
